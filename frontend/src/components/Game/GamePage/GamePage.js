    ...
    streams = () => {
        ...
        if(streams && typeof(streams.error) === 'undefined') {
            return streams.map(stream => {
                ...
                return <a href={'https://twitch.tv/' + stream.user_name} key={stream.user_name} target="_BLANK" rel='noreferrer noopener' className={style.streamCard}>
                    <ReactImage 
                        alt={'Twitch Stream - ' + stream.user_name}
                        src={imageUrl}
                        loader={<div className={style.streamImagePlaceholder}></div>}
                    />
